...
game.js              — вся логика: состояние, DOM, движение лифта, победа
...
| `updateSyllableDisplay()` | Обновляет слог + подсказку (слово + иконка из `syllablesData`) |
...
8. **Высота этажа 55px** жёстко зашита в CSS (.floor height) и JS (updateElevatorPosition). Менять синхронно
